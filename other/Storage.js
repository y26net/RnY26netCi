...
class StorageDB {
  constructor(options = {}) {
    ...
    if (this._s && this._s.setItem) {
      try {
        var promiseTest = this._s.setItem(
          '__react_native_storage_test',
          'test',
        );
  ...
  _lookupGlobalItem(params) {
    ...
    return this.getItem(key).then((ret) =>
      this._loadGlobalItem({ret, ...params}),
    );
  }
  ...
  _lookUpInMap(params) {
    ...
    if (m[newId] !== undefined) {
      return this.getItem('map_' + m[newId]).then((ret) =>
        this._loadMapItem({ret, ...params}),
      );
    }
