...
export const _writeFile = (targetName, content, callback) => {
  ...
  RNFS.writeFile(path, content, 'utf8').then((result) => callback(1));
};
...
export const _appendFile = (fileName, content, callback) => {
  RNFS.appendFile(
    `${ExternalDirectoryPath}/${fileName}`,
    content,
    'utf8',
  ).then(() => callback(1));
};
...
export const _deleteFile = (targetName, callback) => {
  RNFS.unlink(`${ExternalDirectoryPath}/${targetName}`).then(() => callback(1));
};
...
export const _fileEx = (filePath, callback) => {
  RNFS.exists(filePath).then((res) => callback(res));
};
